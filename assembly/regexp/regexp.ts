...
export class RegExp {
  ...
  private groupMarkers: GroupStartMarkerState[] = RegExp.gm;

  constructor(
    private regex: string,
    public flagsString: string | null = null,
  ) {
  ...
  exec(str: string): Match | null {
    ...
    for (
      ...
      if (matchStr != null) {
        ...
        const matchArray: Array<string> = [matchStr];
        const lastCaptures: Array<string> = lastCapturesForGroup(groupMarkers);
        const match = new Match(
          matchArray.concat(lastCaptures),
          matchIndex,
          str,
        );
